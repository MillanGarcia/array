...
//function name[i]{
  ...
  function contarWaldos(str) {
    //Pasamos la cadena a minusculas
    str = str.toLowerCase();
    //quitamos los espacios en blanco
    str = str.replace(/\s/g, "");
    let counts = {}; //Donde guardamos los resultados
    for(let waldo in str){ //Cogemos el indice de cada caracter
      if(str[waldo] in counts) { //Si ya existe, simplemente aumentamos el contador
          counts[str[waldo]] = counts[str[waldo]] + 1
      } else { // Si no existe, lo inicializamos a 1
          counts[str[waldo]] = counts[str[waldo]] 
      }
  }
  ...
  console.log(contarWaldos(people))